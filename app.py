...
def get_path(filename):
    for base in [".", os.path.dirname(os.path.abspath(__file__))]:
        for folder in ["input"]:
            path = os.path.abspath(os.path.join(base, folder, filename))
            if os.path.exists(path):
                return path
    return filename


SCALE_PATH = get_path("aft_scale_parameter.csv")
COEFF_PATH = get_path("aft_coefficient_result.csv")
TRAIN_PATH = get_path("data_loan_dev_80_stratified.csv")
